...
Note：这些只是我自己的主观总结，用于打开设计思路的，很多对GAS的理解可能是错的。
...
1. 定义：`GameplayAbilities` (GA) 是游戏里`Actor`可以使用的技能。同一时间可以同时激活多个`GA`，例如冲刺的同时可以射击。常见的`GA`：跳跃、冲刺、设计、每X秒免疫一次伤害、使用药水、打开一扇门、收集资源、建造建筑。`GA`具有等级，可以通过改变`GA`等级改变`GA`的数值或功能。`GA`使用`AbilityTasks`处理持续一段时间的动作，例如监听某个事件、等待属性改变、等待玩家选择目标等等。
一个简单点的`GA`流程如下：
![](https://johnyoung404.github.io/img/gas/GAS_s.png)
一个复杂一些的`GA`流程如下：
![](https://johnyoung404.github.io/img/gas/GAS_c.png)